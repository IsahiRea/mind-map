...
function NodeDetailsContent({
  ...
  const [editedConnections, setEditedConnections] = useState((connectedNodes || []).map(n => n.id))
  ...
  return (
    <div className="node-details-modal">
      ...
      <div className="node-details-header">
        <div className="node-details-header-content">
          ...
          <div className="node-details-title-section">
            ...
            <div
              className="node-details-topic-badge"
              style={{
      ...
      <p className="node-details-modal-description">
        {isVisitorMode
          ? 'View learning node details'
          : isEditMode
            ? 'Edit your learning node'
            : 'View and edit learning node details'}
      </p>
      ...
      <div className="node-details-content">
        <div className="node-details-section">
          ...
          {isEditMode ? (
            ...
            <p className="node-details-text">{node.description || 'No notes added yet.'}</p>
          )}
        </div>
      ...
      {isEditMode && (
        <div className="node-details-footer">
          <button type="button" className="node-details-cancel-btn" onClick={handleCancel}>
            Cancel
          </button>
          <button
            type="button"
...
export default function NodeDetailsModal({ isOpen, node, ...props }) {
  if (!node) return null

  return (
    <Modal isOpen={isOpen} onClose={props.onClose}>
      <NodeDetailsContent key={node.id} node={node} {...props} />
    </Modal>
  )
}
